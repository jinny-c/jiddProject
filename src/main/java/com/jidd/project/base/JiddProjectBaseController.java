...
import com.jidd.project.common.JiddProjectErrorCode;
 ...
 * @history
 */
public class JiddProjectBaseController {

	public static String CONTENT_TYPE_TEXT = "text/plain; charset=UTF-8";

	/** 日志 */
	private static Logger log = LoggerFactory
			.getLogger(JiddProjectBaseController.class);
	 ...
	 *             没有进行初始化过滤（MicroInitFilter）调用此方法将抛异常
	 */
	protected HttpHeader getHttpHeader() throws JiddControllerException {
		Object header = RequestResponseContext.getRequest().getAttribute(
	...
	protected String toViewHtml(String viewName) {
		return JiddStringUtils.join("view", viewName);
	}
	...
	protected JiddModelAndView<RespSucc> toRequestForward(String url)
		...
		try {
			...
			e.printStackTrace();
			throw new JiddControllerException(
					JiddProjectErrorCode.ERROR_CODE_9999.getErrorCode(),
					JiddProjectErrorCode.ERROR_CODE_9999.getErrorMsg());
		} catch (IOException e) {
			e.printStackTrace();
			throw new JiddControllerException(
					JiddProjectErrorCode.ERROR_CODE_9999.getErrorCode(),
	...
	protected Object getUserBySession() throws JiddControllerException {
		...
		if (userVO == null) {
			log.error("TraceID:{}, 无卡用户会话不存在，请检查拦截器配置或重新进入",
					JiddTraceLogUtil.getTraceId());
			throw new JiddControllerException(ERROR_CODE_MW001.getErrorCode(),
					ERROR_CODE_MW001.getErrorMsg());
		}
		return userVO;
	}
}
