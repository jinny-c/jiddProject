...
class JiddHttpParams {
	/** 通讯URL **/
	private String url;
	/** 通讯编码，默认：UTF-8 **/
	private Charset charset = JiddHttpConstants.DEF_CHARSET;
	/** 连接超时时间,单位:毫秒，默认：10s **/
	private Integer connTimeOut = JiddHttpConstants.DEF_CONN_TIMEOUT;
	/** 响应超时时间,单位:毫秒，默认:10s **/
	private Integer soTimeOut = JiddHttpConstants.DEF_SO_TIMEOUT;
	/** 多线程线程池的最大连接数,默认:5个 **/
	private Integer maxTotal = JiddHttpConstants.DEF_MAX_TOTAL;
	/** 多线程线程池每个域名最大分配连接数,默认:5个 **/
	private Integer maxPerRoute = JiddHttpConstants.DEF_MAX_PER_ROUTE;
	/** 客户端信任证书路径 **/
	...
	Charset getCharset() {
		if (charset == null) {
			return JiddHttpConstants.DEF_CHARSET;
		}
		return charset;
	}
	...
	Integer getConnTimeOut() {
		if (isIntErr(connTimeOut)) {
			return JiddHttpConstants.DEF_CONN_TIMEOUT;
		}
		return connTimeOut;
	}
	...
	Integer getSoTimeOut() {
		if (isIntErr(soTimeOut)) {
			return JiddHttpConstants.DEF_SO_TIMEOUT;
		}
		return soTimeOut;
	}
	...
	Integer getMaxTotal() {
		if (isIntErr(maxTotal)) {
			return JiddHttpConstants.DEF_MAX_TOTAL;
		}
		return maxTotal;
	}
	...
	Integer getMaxPerRoute() {
		if (isIntErr(maxPerRoute)) {
			return JiddHttpConstants.DEF_MAX_PER_ROUTE;
		}
		return maxPerRoute;
	}
	 ...
	 */
	boolean isHttps() {
		if (JiddStringUtils.isBlank(url)) {
			return false;
		}
		return url.trim().toLowerCase()
				.startsWith(JiddHttpConstants.HTTPS_PREFIX);
	}
