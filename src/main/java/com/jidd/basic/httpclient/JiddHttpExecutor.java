...
public class JiddHttpExecutor {
	private static final Logger logger = Logger
			.getLogger(JiddHttpExecutor.class);

	/** 通讯编码，默认:UTF-8 **/
	private Charset charset = JiddHttpConstants.DEF_CHARSET;
	/** 通讯URL，全局共享 **/
	 ...
	 * @param requestConfig
	 */
	JiddHttpExecutor(Charset charset, String url,
	...
	public <T> T doGet(String body, Map<String, String> headMap,
	                   JiddSerialTypeEnum serial, Class<? extends T> clazz)
			throws Exception {
		validate(serial, clazz);
	...
	public String doGetWithUrl(String url, Map<String, String> bodyMap, Map<String, String> headMap)
		...
		for (String key : bodyMap.keySet()) {
			...
			body.append(count > 0 ? JiddSymbolEnum.Ampersand.symbol()
					: JiddSymbolEnum.Blank.symbol());
			body.append(key).append(JiddSymbolEnum.Equals.symbol())
	...
	public String doGetWithUrl(String url, String body, Map<String, String> headMap)
			throws Exception {
		validate(url, body);
		final String realUrl = JiddStringUtils.join(JiddSymbolEnum.QuestionCode, url, body);
	...
	private <T> void validate(JiddSerialTypeEnum e, Class<? extends T> clazz)
			throws Exception {
