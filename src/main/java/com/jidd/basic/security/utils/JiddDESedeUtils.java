package com.jidd.basic.security.utils;
 ...
 * @history
 */
public class JiddDESedeUtils {

	private static Logger logger = LoggerFactory.getLogger(JiddDESedeUtils.class);
	...
	public static String doEncr(String key, String data) {
		try {
			return JiddHexUtil.byte2hex(encr(JiddHexUtil.hex2byte(key), data.getBytes(CHARSET_UTF8)));
		} catch (Exception e) {
	...
	public static String doDecr(String key, String data) {
		try {
			return new String(decr(JiddHexUtil.hex2byte(key), JiddHexUtil.hex2byte(data)),CHARSET_UTF8);
		} catch (Exception e) {
			logger.error("doEncr exception,key={},data={},exception={}", key, data, e);
		}
		return JiddSymbolEnum.Blank.symbol();
